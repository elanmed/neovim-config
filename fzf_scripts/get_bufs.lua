...
--- @type string
local cwd = vim.rpcrequest(chan, "nvim_exec_lua", "return vim.fn.getcwd()", {})
...
for _, buf in pairs(buf_list) do
  ...
  local readable = vim.rpcrequest(chan, "nvim_exec_lua", ("return vim.fn.filereadable(%q)"):format(bname), {})
