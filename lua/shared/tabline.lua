...
local get_name = function(bufnr)
  if vim.bo[bufnr].buftype == "terminal" then
    return terminal_buf_name
  end

  local bufname = vim.api.nvim_buf_get_name(bufnr)
  if bufname == "" then
    return unnamed_buf_name
  end
...
local get_alt_buf_section = function()
  ...
  if get_name(vim.fn.bufnr "%") == unnamed_buf_name then
    return ""
  end
  if get_name(vim.fn.bufnr "%") == terminal_buf_name then
    return ""
  end
  if get_name(bufnr) == get_name(vim.fn.bufnr "%") then
    return ""
  end
...
_G.Tabline = function()
  ...
  local truncated_buf_section =
    h.str.truncate(get_curr_buf_section(), { max_len = section_len, side = "left" })

  local padded_buf_section =
    h.str.pad(truncated_buf_section, { min_len = section_len * 1.5, side = "right" })

  local truncated_alt_section =
    h.str.truncate(get_alt_buf_section(), { max_len = section_len, side = "left" })
